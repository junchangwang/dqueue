...
static void * thread(void * bits)
{
  ...
  //printf("Thread %d to map to core %d\n", id, cpu);
  CPU_SET(cpu, &set);
...
#ifdef VERIFY
    break;  // To verify correctness. Run once.
#endif
  }
...
int main(int argc, const char *argv[])
{
  ...
  /**
   * The third argument is workload size, if specified.
   */
#ifdef WORKLOAD
  if (argc > 3) {
  ...
  #ifdef VERIFY
    printf("  VERIFY specified. Perform evaluation only once.\n");
    printf("  run './verify <director containing log files generated>' to verify correctness.\n");
  #endif
...
#ifdef WORKLOAD
  printf("  Performance: %ld - %d = %ld ns/op\n", ((long)mean*1000000)/nops, workload, ((long)mean*1000000)/nops-workload);
#endif
  printf("===========================================\n");
